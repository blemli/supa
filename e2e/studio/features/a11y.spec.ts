...
// Self-hosted-reachable, structurally distinct routes. Platform-only surfaces
// (`/org`, `/account`, billing, branching) render null or redirect here, and
// `/logs/*` needs a mocking decision before its skeleton states mean anything.
const ROUTES = [
...
// The shell is identical on every route, so it is scanned once rather than ten
// times. The project home page is the cheapest route to render it on.
const SHELL_REGIONS = [
...
// Four of the scanned routes render an empty state unless the project has data,
// and an empty route that baselines at zero is how a ratchet silently stops
// working. Each seed is idempotent so a rerun after a failed cleanup still works.
...
async function reportScan(testInfo: TestInfo, result: AxeArtifact, scope: string) {
...
test.describe('Studio accessibility scan', () => {
  ...
  for (const { surface, path } of ROUTES) {
    test(`${surface} has no new accessibility violations @a11y`, async ({
      ...
      // The shell returns null until it mounts, so without settling every scan
